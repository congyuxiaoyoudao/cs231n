...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    # text tokenize and embedding
    text_tokens = clip.tokenize(class_texts).to(device)
    text_features = clip_model.encode_text(text_tokens)

    # pre-process image and embedding
    processed_images = [
    ...
    # pass image and text features to the model
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # pre-process and embedding images here to avoid repeating procedure
        processed_images = [
            clip_preprocess(Image.fromarray(img)).unsqueeze(0)
            for img in images
        ]
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_features = self.model.encode_text(text_tokens) # (1, D)
        
        text_features_norm = text_features / torch.linalg.norm(text_features,dim=1,keepdim=True)
        image_features_norm = self.image_features_norm # (M, D)

        similarity = text_features_norm @ image_features_norm.T # (1, M)
